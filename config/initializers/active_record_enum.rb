  ...
  module Enum
    def enum(definitions)
      ...
      definitions.each { |name, values|
        ...
        _enum_methods_module.module_eval {
          # Example:
          #   def status=(value)
          ...
          define_method("#{name}=") { |value|
            case
              when enum_values.has_key?(value) || value.blank?
                self[name] = enum_values[value]
              when enum_values.has_value?(value)
    ...
    def enum_hash
      ...
      if klass.defined_enums.present?
        klass.defined_enums.keys.each { |enum_name|
          plural_name = "#{enum_name}".pluralize

          klass.singleton_class.send(:define_method, "#{plural_name}_by_sym") { |*values|
            if values.any? { |val| val.is_a?(Array) }
              raise ArgumentError, 'Values should be String or Symbol'
            end
